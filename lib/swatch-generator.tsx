...
export function createSwatches({ color }: { color: string }): {
  ...
  const palette = swatches.filter(
    (swatch) => swatch.stop !== 0 && swatch.stop !== 1000
  )
...
export function useShadesStyles({ palette }: { palette: PalleteSwatch[] }): {
  shadesStyles: React.CSSProperties
} {
