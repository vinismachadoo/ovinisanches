...
interface OrderProviderAvatarProps extends React.ComponentPropsWithoutRef<
  typeof Avatar
> {
  order_integration_type: string
}

const OrderProviderAvatar = React.forwardRef<
  React.ElementRef<typeof Avatar>,
  OrderProviderAvatarProps
>(({ order_integration_type, className, ...props }, ref) => {
  ...
  return (
    <Avatar
      ...
      {...props}
      className={cn(
        "h-6 w-6 cursor-default border border-border/50 shadow-md select-none",
        className
      )}
    >
      <AvatarImage src={order_provider_logo} />
      <AvatarFallback className="text-xs">
        {order_integration_type?.slice(0, 2)?.toUpperCase()}
      </AvatarFallback>
    </Avatar>
  )
})
