...
const PresentationProvider = ({ children }: React.PropsWithChildren) => {
  ...
  return (
    <PresentationContext.Provider
      ...
      value={{ api, setApi, current, setCurrent, fullscreen, fullscreenRef, toggleFullscreen }}
    >
      {children}
    </PresentationContext.Provider>
  );
...
const PresentationContent = ({ children, className, ...props }: React.ComponentProps<typeof CarouselContent>) => {
  return (
    <CarouselContent data-slot="presentation-content" className={cn('h-full ml-0', className)} {...props}>
      {children}
    </CarouselContent>
  );
};
...
const PresentationSlide = ({
  ...
  return (
    ...
    >
      <div className={cn('flex items-center justify-center size-full', className)} {...props}>
        {children}
      </div>
    </CarouselItem>
  );
...
const PresentationWatermark = ({ children, className, ...props }: React.ComponentProps<'div'>) => {
  return (
    <div
      data-slot="presentation-watermark"
      className={cn('absolute bottom-6 left-1/2 -translate-x-1/2 text-muted-foreground', className)}
      {...props}
    >
      {children}
    </div>
...
const PresentationControls = ({ children, className, ...props }: React.ComponentProps<'div'>) => {
  const { api, current, setCurrent, fullscreen, toggleFullscreen } = usePresentation();
  const { resolvedTheme, setTheme } = useTheme();
