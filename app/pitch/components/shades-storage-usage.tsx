          ...
          <div className="flex flex-col gap-y-2">
            ...
            <div className="flex h-4 w-full overflow-hidden">
              {data.map(({ source, color, usage }) => (
                <div
                  key={source}
                  className="flex w-(--width) items-center not-last:pr-1"
                ...
                >
                  <div
                    className={cn(
                      "h-full w-full rounded-xs bg-(--bg-color)",
            ...
            {data.map(({ source, color, usage }, idx) => (
              <div
                ...
                className={cn(
                  ...
                  hoveredSource && hoveredSource === source && "bg-muted"
                )}
                onMouseEnter={() => setHoveredSource(source)}
                onMouseLeave={() => setHoveredSource(null)}
              >
                ...
                >
                  <div className="flex items-center gap-x-2 text-sm">
                    <span
                      className={cn("size-3 shrink-0 rounded-xs bg-(--color)")}
                      style={
                        {
                          "--color": color,
                        } as React.CSSProperties
                    ...
                    />
                    <span className="whitespace-nowrap text-muted-foreground select-all">
                      {source}
                    </span>
                  </div>
