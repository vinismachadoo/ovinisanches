...
const expenseSchema = z
  .object({
    ...
    shareMemberIds: z
      .array(z.string())
      .min(1, "At least one member must share the expense"),
  ...
  const loadExpenseData = async () => {
    ...
    try {
      ...
      let payerSplitType: "even" | "percentage" | "amount" =
        (expense as Expense & { paid_type: "even" | "percentage" | "amount" })
          .paid_type || "even"
      ...
      if (
        !(expense as Expense & { paid_type?: string }).paid_type &&
        payers.length > 0
      ) {
        const payerAmountsList = payers.map((p) =>
          parseFloat(p.amount.toString())
        )
        const firstAmount = payerAmountsList[0]
        const allEqual = payerAmountsList.every(
          (a) => Math.abs(a - firstAmount) < 0.01
        )
        ...
        if (allEqual) {
          ...
          const percentages = payerAmountsList.map(
            (a) => (a / totalAmount) * 100
          )
          const roundedPercentages = percentages.map(
            (p) => Math.round(p * 100) / 100
          )
          const sumPercentages = roundedPercentages.reduce(
            (sum, p) => sum + p,
            0
          )
        ...
        if (payerSplitType === "percentage") {
          payers.forEach((p) => {
            const percentage =
              (parseFloat(p.amount.toString()) / totalAmount) * 100
      ...
      if (expense.split_type === "percentage") {
        shares.forEach((share) => {
          const percentage =
            (parseFloat(share.amount.toString()) / totalAmount) * 100
  ...
  const handlePayerIdsChange = (memberIds: string[]) => {
    ...
    if (payerSplitType === "percentage") {
      ...
      memberIds.forEach((id) => {
        newPercentages[id] = memberIds.includes(id)
          ? payerPercentages?.[id] || evenPercent
          : ""
      })
      ...
      memberIds.forEach((id) => {
        newAmounts[id] = memberIds.includes(id)
          ? payerAmounts?.[id] || evenAmount
          : ""
      })
  ...
  if (loadingData) {
    return (
      <Dialog open={open} onOpenChange={onOpenChange}>
        <DialogContent>
          <div className="py-4 text-center">Loading expense data...</div>
        </DialogContent>
      </Dialog>
    )
  }
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <FormControl>
                    <Input
                      placeholder="e.g., Groceries, Dinner, Hotel"
                      {...field}
                    />
                  </FormControl>
                  <FormMessage />
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <FormDescription>
                      Select who paid for this expense (can be multiple)
                    </FormDescription>
                  ...
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        ...
                        <FormDescription>
                          {payerSplitType === "even" &&
                            "Each payer paid an equal share"}
                          {payerSplitType === "percentage" &&
                            "Enter percentage for each payer (must total 100%)"}
                          ...
                          <span>
                            {Object.values(payerPercentages || {})
                              .reduce(
                                (sum, p) =>
                                  sum +
                                  (isNaN(parseFloat(p)) ? 0 : parseFloat(p)),
                                0
                              )
                              .toFixed(2)}
                            %
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <FormDescription>
                    Select who should share this expense (can be multiple)
                  </FormDescription>
                  <div className="mt-2 space-y-2">
                    {members.map((member) => (
                      <div
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <FormDescription>
                    {splitType === "even" &&
                      "The expense will be split equally among selected members"}
                    {splitType === "percentage" &&
                      "Enter percentage for each member (must total 100%)"}
                    {splitType === "amount" &&
                      "Enter exact amount for each member (must total expense amount)"}
                  </FormDescription>
                  <FormMessage />
                </FormItem>
              )}
                  ...
                  <div className="flex items-center justify-between text-sm font-medium">
                    ...
                    <span>
                      {Object.values(percentages || {})
                        .reduce(
                          (sum, p) =>
                            sum + (isNaN(parseFloat(p)) ? 0 : parseFloat(p)),
                          0
                        )
                        .toFixed(2)}
                      %
                    ...
                    <span>
                      {formatCurrency(
                        Object.values(amounts || {}).reduce(
                          (sum, a) =>
                            sum + (isNaN(parseFloat(a)) ? 0 : parseFloat(a)),
                          0
                        ),
                        form.watch("currency")
                      )}
