...
const DashboardPage = () => {
  ...
  const [selectedParticipantId] = useQueryState(
    "selectedParticipantId",
    parseAsString.withDefault("")
  )

  const [selectedServerId] = useQueryState(
    "selectedServerId",
    parseAsString.withDefault("")
  )

  const [hasEnrollmentsResourceStr] = useQueryState(
    "hasEnrollmentsResource",
    parseAsString.withDefault("")
  )
  const hasEnrollmentsResource =
    hasEnrollmentsResourceStr === ""
      ? null
      : hasEnrollmentsResourceStr === "true"

  const [hasPaymentsConsentsResourceStr] = useQueryState(
    "hasPaymentsConsentsResource",
    parseAsString.withDefault("")
  )
  const hasPaymentsConsentsResource =
    hasPaymentsConsentsResourceStr === ""
      ? null
      : hasPaymentsConsentsResourceStr === "true"

  const [hasPaymentsPixResourceStr] = useQueryState(
    "hasPaymentsPixResource",
    parseAsString.withDefault("")
  )
  const hasPaymentsPixResource =
    hasPaymentsPixResourceStr === ""
      ? null
      : hasPaymentsPixResourceStr === "true"

  const [supportsDCRStr] = useQueryState(
    "supportsDCR",
    parseAsString.withDefault("")
  )
  const supportsDCR = supportsDCRStr === "" ? null : supportsDCRStr === "true"

  const [supportsRedirectStr] = useQueryState(
    "supportsRedirect",
    parseAsString.withDefault("")
  )
  const supportsRedirect =
    supportsRedirectStr === "" ? null : supportsRedirectStr === "true"

  const selectedServer = data
    .find((item: Participant) =>
      item.AuthorisationServers?.find(
        (server: AuthorisationServer) =>
          server.AuthorisationServerId === selectedServerId
      )
    )
    ?.AuthorisationServers?.find(
      (server: AuthorisationServer) =>
        server.AuthorisationServerId === selectedServerId
    )
  ...
  const applyFilters = React.useCallback(() => {
    let newData = data
      .map(
        (item: Participant) =>
          item.AuthorisationServers?.map(
            (server: AuthorisationServer) => server
          ) ?? []
      )
      .flat()
    if (selectedParticipantId) {
      newData = newData.filter(
        (server: AuthorisationServer) =>
          server.OrganisationId === selectedParticipantId
      )
    }
    if (selectedServerId) {
      newData = newData.filter(
    ...
    if (hasEnrollmentsResource !== null) {
      newData = newData.filter(
        (server: AuthorisationServer) =>
          supportsContactlessPix(server).hasEnrollmentsResource ===
          hasEnrollmentsResource
      )
    }
    if (hasPaymentsConsentsResource !== null) {
      newData = newData.filter(
        (server: AuthorisationServer) =>
          supportsContactlessPix(server).hasPaymentsConsentsResource ===
          hasPaymentsConsentsResource
      )
    }
    if (hasPaymentsPixResource !== null) {
      newData = newData.filter(
        (server: AuthorisationServer) =>
          supportsContactlessPix(server).hasPaymentsPixResource ===
          hasPaymentsPixResource
      )
    }
    if (supportsDCR !== null) {
      newData = newData.filter(
        (server: AuthorisationServer) =>
          supportsContactlessPix(server).supportsDCR === supportsDCR
      )
    }
    if (supportsRedirect !== null) {
      newData = newData.filter(
        (server: AuthorisationServer) =>
          supportsContactlessPix(server).supportsRedirect === supportsRedirect
      )
    }
    return newData
  }, [
  ...
  return (
    <div className="flex h-dvh w-full min-w-0 flex-col justify-center">
      <div className="relative flex h-full min-h-0 w-full min-w-0 flex-1 flex-col items-start">
        <div className="flex h-full w-full flex-col">
          <div className="p-4">
            <h1 className="text-2xl font-bold">
              Open Banking - Contactless PIX - Raidiam Case
            </h1>
          </div>
          ...
          <div className="flex h-full min-h-0 flex-col gap-y-4 p-4">
            <FilterSection data={data} />

            <div className="flex h-full min-h-0 gap-x-4">
              <div className="flex min-h-0 w-full flex-col gap-y-4">
                <span className="font-medium">
                  {filteredData.length} results
                </span>

                <div className="grid h-fit min-h-0 grid-cols-3 items-start gap-4 overflow-auto">
              ...
              <div className="flex h-full max-h-full min-h-0 w-full max-w-full min-w-0 overflow-auto rounded-sm border p-2">
                {selectedServer ? (
                  <pre>
...
const MetricsSection = ({ data }: MetricsSectionProps) => {
  ...
  const activeParticipantsAmount = data.filter(
    (item: Participant) => item.Status === "Active"
  ).length

  // authorization servers
  const listOfServers = data
    .map((item: Participant) => item.AuthorisationServers)
    .flat()
  ...
  const serversWithSupportToContactlessPix = listOfServers.filter(
    (server: AuthorisationServer | undefined) =>
      server &&
      supportsContactlessPix(server, DEFAULT_REQUIREMENTS_TO_VALIDATE)
        .hasAllRequirements
  )
  const participantsWithSupportToContactlessPix = [
    ...new Set(
      serversWithSupportToContactlessPix.map(
        (server: AuthorisationServer | undefined) => server?.OrganisationId
      )
    ),
  ]
  ...
  const serversWithSupportToPaymentsPix = listOfServers.filter(
    (server: AuthorisationServer | undefined) =>
      server && checkPaymentsPixResource(server)
  )
  const participantsWithSupportToPaymentsPix = [
    ...new Set(
      serversWithSupportToPaymentsPix.map(
        (server: AuthorisationServer | undefined) => server?.OrganisationId
      )
    ),
  ]

  const serversWithSupportToPaymentsPixAmount =
    serversWithSupportToPaymentsPix.length

  return (
    <div className="flex justify-between gap-x-4 p-4">
      <div className="flex h-fit w-fit max-w-80 flex-col gap-2 rounded-sm border border-l-3 border-l-orange-400 p-4">
      ...
      <div className="flex h-fit w-fit max-w-md flex-col gap-2 rounded-sm border border-l-3 border-l-orange-400 p-4">
        <div className="flex items-center gap-2">
          <p className="text-muted-foreground">
            Servers with support to pix payments
          </p>
          <p className="text-lg font-bold">
            {serversWithSupportToPaymentsPixAmount}
          </p>
        </div>
        <Badge>
          from {participantsWithSupportToPaymentsPix.length} participants
        </Badge>
      </div>

      <div className="flex h-fit w-fit max-w-160 flex-col gap-2 rounded-sm border border-l-3 border-l-orange-400 p-4">
        <div className="flex items-center gap-2">
          <p className="text-muted-foreground">
            Servers with support to contactless pix
          </p>
          <p className="text-lg font-bold">
            {serversWithSupportToContactlessPix.length}
          </p>
        </div>

        <Badge>
          from {participantsWithSupportToContactlessPix.length} participants
        </Badge>
      </div>
      ...
      <div className="flex h-fit w-fit max-w-160 flex-col gap-2 rounded-sm border border-l-3 border-l-orange-400 p-4">
        ...
        <div className="flex flex-wrap gap-2">
          {["enrollments", "payments-consents", "payments-pix"].map(
            (requirement) => (
...
const FilterSection = ({ data }: { data: Participant[] }) => {
  ...
  const [selectedServerId, setSelectedServerId] = useQueryState(
    "selectedServerId",
    parseAsString.withDefault("")
  )
  ...
  const [hasPaymentsConsentsResource, setHasPaymentsConsentsResource] =
    useQueryState("hasPaymentsConsentsResource", parseAsString.withDefault(""))
  const [hasPaymentsPixResource, setHasPaymentsPixResource] = useQueryState(
    "hasPaymentsPixResource",
    parseAsString.withDefault("")
  )
  const [supportsDCR, setSupportsDCR] = useQueryState(
    "supportsDCR",
    parseAsString.withDefault("")
  )
  const [supportsRedirect, setSupportsRedirect] = useQueryState(
    "supportsRedirect",
    parseAsString.withDefault("")
  )
  ...
  return (
    <div className="flex gap-x-2">
      <div className="flex flex-col gap-y-2 rounded-sm border p-2">
...
const AuthServerCard = ({ server }: { server: AuthorisationServer }) => {
  const [, setSelectedParticipantId] = useQueryState(
    "selectedParticipantId",
    parseAsString.withDefault("")
  )
  const [selectedServerId, setSelectedServerId] = useQueryState(
  ...
  const supportsContactlessPixResult = supportsContactlessPix(
    server,
    REQUIREMENTS_TO_SHOW_IN_CARD
  )
  ...
  return (
    <div
      className={cn(
        "relative flex h-full w-full max-w-80 flex-col gap-2 rounded-sm border p-4",
    ...
    >
      <div className="flex items-center gap-2">
        <Image
          ...
          alt={server.CustomerFriendlyName}
          width={20}
          height={20}
        />
          ...
          REQUIREMENTS_TO_SHOW_IN_CARD.map((requirement) => {
            return (
              <div key={requirement} className="flex items-center gap-2">
                <p>
                  {supportsContactlessPixResult[
                    requirement as keyof typeof supportsContactlessPixResult
                  ] ? (
      ...
      {selectedServerId === server.AuthorisationServerId && (
        <Button
          size="sm"
          className="absolute top-2 right-2 w-fit self-end rounded-full"
          onClick={() => {
            setSelectedParticipantId("")
            setSelectedServerId("")
          }}
