...
export function KnockoutBracket({ groupStandings }: KnockoutBracketProps) {
    ...
    if (team.isPlayoff) {
      return (
        <span
          className={`${
            size === "sm" ? "text-sm" : size === "lg" ? "text-5xl" : "text-base"
          } flex h-4 w-6 items-center justify-center rounded bg-muted text-xs font-bold`}
        >
          ?
        </span>
      )
  ...
  const MatchSlot = ({
    ...
    return (
      <div
        className={`rounded border border-border bg-card ${compact ? "text-xs" : "text-sm"}`}
      >
        <div
          className={`flex items-center gap-2 border-b border-border/50 p-2 ${
        ...
        >
          ...
          <Input
            ...
            className={`${compact ? "h-6 w-8 text-xs" : "h-7 w-10 text-sm"} bg-background p-0 text-center font-bold`}
            disabled={!team1}
          />
        </div>
        <div
          className={`flex items-center gap-2 p-2 ${winner === match.team2.code ? "bg-success/20" : ""}`}
        >
          ...
          <Input
            ...
            onChange={(e) =>
              updateScore(round, matchIdx, false, e.target.value)
            }
            className={`${compact ? "h-6 w-8 text-xs" : "h-7 w-10 text-sm"} bg-background p-0 text-center font-bold`}
            disabled={!team2}
          />
  ...
  return (
    <div className="space-y-6">
      {/* Champion Display */}
      {champion && (
        <Card className="from-gold/20 to-gold/20 border-gold/50 bg-linear-to-r via-accent/20">
          <CardContent className="p-6 text-center">
            <Trophy className="text-gold mx-auto mb-4 h-16 w-16" />
            <h2 className="mb-2 text-2xl font-bold">WORLD CHAMPION</h2>
        ...
        <div className="min-w-[1200px]">
          <div className="grid grid-cols-9 items-center gap-4">
            ...
            <div className="space-y-2">
              <h3 className="mb-3 text-center text-xs font-bold text-muted-foreground">
                ROUND OF 32
              </h3>
            ...
            {/* QF + SF + Final */}
            <div className="space-y-6">
              <h3 className="text-center text-xs font-bold text-muted-foreground">
                QUARTER-FINALS
              </h3>
              <div className="space-y-4">
              ...
              <h3 className="mt-6 text-center text-xs font-bold text-muted-foreground">
                SEMI-FINAL
              </h3>
              <MatchSlot match={sf[0]} round="sf" matchIdx={0} />

              <h3 className="mt-6 text-center text-xs font-bold text-primary">
                🏆 FINAL
              </h3>
              <div className="from-gold/10 to-gold/10 rounded-lg bg-linear-to-r p-1">
