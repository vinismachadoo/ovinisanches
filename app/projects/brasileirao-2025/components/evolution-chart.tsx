...
import { ChartContainer, ChartTooltip } from "@/registry/default/ui/chart"
...
export function EvolutionChart() {
  const [selectedTeams, setSelectedTeams] = useQueryState(
    "teams",
    parseAsArrayOf(parseAsString).withDefault([])
  )
  const [visibleZones, setVisibleZones] = useQueryState(
    "zones",
    parseAsArrayOf(parseAsString).withDefault([])
  )
  ...
  const allReferenceAreaCodes = React.useMemo(
    () => Object.keys(REFERENCE_AREAS) as ReferenceAreaCode[],
    []
  )
  ...
  return (
    <div className="flex h-full min-h-0 flex-col gap-y-4">
        ...
        {allReferenceAreaCodes.map((zone) => {
          ...
          return (
            ...
            >
              <span
                className={cn(
                  "inline-block h-1 w-4 rounded-sm border border-(--border-color) bg-(--bg-color)"
                )}
                style={
                  {
      ...
      <ChartContainer config={chartConfig} className="h-full min-h-0 w-full">
        ...
        >
          ...
          <ReferenceArea
            ...
            fill={
              isZoneVisible("LIB")
                ? `hsl(from ${referenceAreaColors["LIB"]} h s 75%)`
                : "transparent"
            }
          />
          ...
          <ReferenceArea
            ...
            fill={
              isZoneVisible("LIB2")
                ? `hsl(from ${referenceAreaColors["LIB2"]} h s 75%)`
                : "transparent"
            }
          />
          <ReferenceArea
            ...
            fill={
              isZoneVisible("SUL")
                ? `hsl(from ${referenceAreaColors["SUL"]} h s 75%)`
                : "transparent"
            }
          />
          <ReferenceArea
            ...
            fill={
              isZoneVisible("REB")
                ? `hsl(from ${referenceAreaColors["REB"]} h s 75%)`
                : "transparent"
            }
          />
          <XAxis
          ...
          <ChartTooltip
            content={({ active, payload, label }) => {
              ...
              const selectedTeams = payload.filter((entry) =>
                isTeamSelected(entry.dataKey as TeamCode)
              )
                      ...
                      .map((entry, index) => {
                        ...
                        return (
                          <div key={index} className="flex items-center gap-2">
                            ...
                            />
                            <span
                              className={`text-sm ${selected ? "" : "opacity-50"}`}
                            >
      ...
      {/* Legend */}
      <div className="grid grid-cols-10 justify-center gap-3">
        {allTeams.map((team) => {
          const selected = isTeamSelected(team)
          return (
            <Button
